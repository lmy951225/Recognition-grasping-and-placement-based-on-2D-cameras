...
from Sisyphe.driver.grasp.gripper2_test import ZXHand
from Sisyphe.point.client import Pt,GraspPlanningClient 
...
class Client():
    ...
    def test_BiasMovePose(self, bias_x = 0, bias_y = 0, bias_z = 0, bias_rx = 0, bias_ry = 0, bias_rz = 0):
        x, y, z, a, b, c = self.test_getPose()
        print(x, y, z, a, b, c)
        req = pb2.CartesianPosition(x=x+bias_x,
                                    y=y+bias_y,
                                    z=z+bias_z,
                                    rx=a+bias_rx,
                                    ry=b+bias_ry,
    ...
    def test_MovePose(self, x,y,z,rx,ry,rz):
    ...
    def test_movePose_grasp(self,tcp_x,tcp_y,tcp_z,tcp_rx,tcp_ry,tcp_rz):
        x, y, z, a, b, c = self.test_getPose()
        print(x, y, z, a, b, c)
        req = pb2.CartesianPosition(x=tcp_x,
                                    y=tcp_y,
                                    z=tcp_z,
                                    rx=tcp_rx,
                                    ry=tcp_ry,
...
if __name__ == '__main__':
    ...
    tcp_client = GraspPlanningClient()
    Pt1 = Pt(x=571.610046, y=10.440644, z = 222.961029)
    Pt2 = Pt(x=571.565735 ,y =-10.549973,z = 219.105759)

    x,y,z,rx,ry,rz = tcp_client.client_get2Points(Pt1=Pt1,Pt2=Pt2)
    ...
    c.test_connect()
    c.test_movePose_grasp(x,y,z,rx,ry,rz)
    time.sleep(2)
    lmy.close()
    time.sleep(2)
    c.test_BiasMovePose(bias_z = 100)
    c.test_BiasMovePose(bias_rx = -50)
    c.test_BiasMovePose(bias_rx = 50)
    c.test_movePose_grasp(x,y,z,rx,ry,rz)
    time.sleep(2)
    lmy.open()
    time.sleep(2)
    c.test_MovePose(x=285.794,y=0.072, z=680.539, rx=178.691,ry=2.356,rz=0.272)
